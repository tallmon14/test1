...
## ROI & TCO
{roi.render_section(roi.default_inputs())}
...
def apply_roi(name, scenario=None, sets=None):
    """Recompute the ROI & TCO section from its inputs. Returns (inputs, results)."""
    text = read(name)
    if text is None:
        return None, None
    sections = parse_sections(text)
    display = display_name(text, name)
    inputs = roi.parse_inputs(sections.get("ROI & TCO", ""))
    if scenario:
        inputs["scenario"] = roi.coerce("scenario", scenario)
    if sets:
        for key, val in sets.items():
            if key in roi.INPUT_KEYS:
                inputs[key] = roi.coerce(key, val)
    results = roi.compute(inputs)
    sections["ROI & TCO"] = roi.render_section(inputs, results)
    new_text = set_field(render(display, sections), "Last updated", today())
    write(name, new_text)
